...
const BeehiveManagement = () => {
  const [hiveGroups, setHiveGroups] = useState([{ id: 1, hives: [] }]);
  ...
  const addHive = (groupId, hexIndex) => {
    setHiveGroups((currentGroups) => {
      return currentGroups.map((group) => {
        ...
        const totalHives = currentGroups.reduce(
          (sum, g) => sum + g.hives.length,
          0
        );
        ...
        <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-lg">
          ...
          <div className="mb-4">
            ...
            <p className="text-lg text-gray-600 italic mt-4">
              Additional hive information would appear here.
            </p>
          </div>
          <button
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-4">
      ...
      <p className="text-center mb-6 text-gray-700">
        Click on a hexagon to add a new hive or view details
      </p>
            ...
            <div key={group.id} className="relative mb-16">
              <div className="honeycomb-grid">
                ...
                <div className="center-hex">
                  {(() => {
                    const hexIndex = 0;
                    const existingHive = group.hives.find(
                      (hive) => hive.position === hexIndex
                    );
                    ...
                    return (
                      <Hexagon
                        ...
                        onClick={() =>
                          existingHive
                            ? selectHive(existingHive)
                            : addHive(group.id, hexIndex)
                        }
                      />
                    );
                  })()}
                ...
                <div className="surrounding-hexes">
                  {[1, 2, 3, 4, 5, 6].map((index) => {
                    const existingHive = group.hives.find(
                      (hive) => hive.position === index
                    );

                    // Calculate position based on index (60 degrees apart)
                    // Added extra distance (95px instead of 80px) for more margin
      ...
      <div className="mt-8 text-center p-4 bg-white rounded-lg shadow max-w-md mx-auto">
        ...
        <ul className="text-left pl-6 list-disc">
          <li className="mb-1">Click on an empty hexagon to add a new hive</li>
          <li className="mb-1">
            Click on a filled hexagon to view hive details
          </li>
          <li className="mb-1">
            When a group is full, a new group can be added
          </li>
          <li>
            Total hives:{" "}
            {hiveGroups.reduce((sum, group) => sum + group.hives.length, 0)}
          </li>
        </ul>
      </div>
